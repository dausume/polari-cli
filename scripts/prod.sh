...
render_stack() {
    ...
    docker compose -f "$(compose_file)" --env-file "$(env_file)" config 2>"$GEN/compose-config.err" \
...
EOF
    sed "s/\${PROD_DOMAIN}/$D/g" "$SUITE/pol-proxy/nginx.prod.conf.template" > "$GEN/nginx.prod.conf"
